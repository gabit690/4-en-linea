...
void MapaDeBits::asignar(unsigned int columna, unsigned int fila,
						 Color* nuevoColor){

	this->imagen->SetPixel(columna,
						   (this->alto - 1) - fila,
						   nuevoColor->obtenerPixel());
}
...
void MapaDeBits::escribir(std::string nombreDeArchivo, unsigned int version){
	...
	std::string nombreDeArchivoConVersion = nombreDeArchivo +
											unNumero.str() +
											".bmp";
